...
const PiercingPriceList = () => {
  ...
  return (
    <section className="container mx-auto px-4 py-8">
      <h2 className="text-5xl md:text-6xl lg:text-7xl xl:text-8xl 2xl:text-9xl text-center font-bold mb-10">
        {priceList.title}
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {priceList.categories.map((category, categoryIndex) => (
          <div
            key={categoryIndex}
            className="backdrop-blur-sm rounded-lg shadow-2xl p-8 border border-gray-700"
          >
            ...
            <ul className="space-y-2">
              {category.items.map((item, itemIndex) => (
                <li
                  key={itemIndex}
                  className="flex justify-between items-baseline text-lg"
                >
                  <span className="flex-grow pr-4 font-medium">
                    {item.name}
                  </span>
